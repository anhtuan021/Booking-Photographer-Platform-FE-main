...
async function proxy(req) {
  try {
    ...
    req.headers.forEach((v, k) => {
      if (k.toLowerCase() !== 'origin') headers.set(k, v);
    });
    let body = undefined;
    ...
    const targetUrl = `${BACKEND}/api/v1/${apiPath}`;
  ...
  return new NextResponse(resText, { status: res.status, headers: resHeaders });
    ...
    return NextResponse.json({ message: err.message || 'Proxy error' }, { status: 500, headers: corsHeaders });
  }
}
